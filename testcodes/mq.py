...
class MQ():
    ...
    # Points taken from curve based on MQ-X Datasheet 
    # Two points were taken to simulate a straight line
    ...
    def MQResistanceCalculation(self, raw_adc):
        return float(self.RL_VALUE*(1023.0-raw_adc)/float(raw_adc)); # OHM's LAW: Sensor Resitance = (Vc - Vs)/Vs
                                                                     # Where: Vc = Circuit Voltage
                                                                     #        Vs = Sensor Voltage
    ...
    ######################## MQPercentage Calculation ###########################
    
    def MQPercentage(self):
        val = {}
        read = self.MQRead(self.MQ_PIN)                                             # MQRead = Sensor Resistance based on Gas
        val["GAS_LPG"]  = self.MQGetGasPercentage(read/self.Ro, self.GAS_LPG)       # read value divided by LPG Ro 
        val["CO"]       = self.MQGetGasPercentage(read/self.Ro, self.GAS_CO)        # read value divided by CO Ro
        val["SMOKE"]    = self.MQGetGasPercentage(read/self.Ro, self.GAS_SMOKE)     # read value divided by Smoke Ro
        return val
